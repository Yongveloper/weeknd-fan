...
8. **본문 링크 캡션은 정본이 쓴 만큼만 쓴다.** `커뮤니티 후기` → `Community review`.
   URL 도메인을 보면 DCInside인지 클리앙인지 알 수 있고 그게 정확하더라도, 정본
   저자는 게시판을 지목하지 않았다. 인접 필드에서 끌어온 정확한 구체성을 한 번
   허용하면 그 다음부터는 선이 없다. 플랫폼 이름은 URL과 `/sources/` 페이지가
   지고 있다 — Task 9의 출처 레코드 `name` 은 반대로 플랫폼을 **밝혀야** 한다.
   그 규칙은 저 레코드들 것이지 본문 캡션 것이 아니다.
   확인된 대응: 오마이뉴스 → OhmyNews, 다음뉴스 → Daum News, 나무위키 → Namuwiki,
   관람 후기 → Concert review.
