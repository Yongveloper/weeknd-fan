...
description: >
  ...
  Bebas Neue Header in lowercase weight 900 treated as graphic primitive, IBM Plex Mono chrome (uppercase,
...
typography:
  # Site override: Korean display text uses Noto Sans KR weight 800 (site --font-display-ko); Bebas Neue has no Hangul.
  # — reading ramp —
  body: { fontFamily: "Noto Sans KR", cqw: 1.2, weight: 400, lineHeight: 1.6 }
  lead: { fontFamily: "Noto Sans KR", cqw: 1.6, weight: 400, lineHeight: 1.5 }
  caption: { fontFamily: "Noto Sans KR", cqw: 0.9, weight: 400, lineHeight: 1.5 }
  label:   { fontFamily: "Noto Sans KR", cqw: 0.72, weight: 700, tracking: "0.14em", upper: true }
  # — display / hero ramp (Bebas Neue for Latin, uppercase as on the site; Korean headlines use Noto Sans KR 800) —
  h3:      { fontFamily: "Bebas Neue", cqw: 2.8, weight: 700, lineHeight: 1.2 }
  quote-text:{ fontFamily: "Bebas Neue", cqw: 3.8, weight: 700, lineHeight: 1.15, tracking: "-0.02em" }
  h2:      { fontFamily: "Bebas Neue", cqw: 4.5, weight: 700, lineHeight: 1.1, tracking: "-0.02em" }
  stat-value:{ fontFamily: "Bebas Neue", cqw: 5.5, weight: 800, lineHeight: 1.0, tracking: "-0.04em" }
  h1:      { fontFamily: "Bebas Neue", cqw: 7.5, weight: 800, lineHeight: 0.9, tracking: "-0.03em" }
  fadelist-item:{ fontFamily: "Bebas Neue", cqw: 7.5, weight: 800, lineHeight: 1.0, tracking: "-0.03em" }
  quote-mark:{ fontFamily: "Bebas Neue", cqw: 10.0, weight: 800, lineHeight: 0.6 }
  fadelist-title:{ fontFamily: "Bebas Neue", cqw: 10.5, weight: 800, lineHeight: 0.9, tracking: "-0.04em" }
  display: { fontFamily: "Bebas Neue", cqw: 13.0, weight: 800, lineHeight: 0.88, tracking: "-0.04em" }
...
- **Fonts** — already set to **Bebas Neue Header** (display) / **Noto Sans KR Header** (body); ignore any preset font name lingering in prose.
- **Weights** — the brand font ships `{400, 450, 700, 750, 800}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
...
text and becomes graphic primitive.** Bebas Neue Header `display` at 13cqw puts a single lowercase word
...
**Bebas Neue Header** carries every text role from display to body — expressive range from weight (400–900)
...
- **Massive lowercase Bebas Neue Header 900**, negative-tracked, as graphic primitive (display 13cqw).
...
Two ramps. The **reading ramp** (Bebas Neue Header body 1.2cqw, mono label 0.72cqw) carries copy + chrome; the
**display ramp** (Bebas Neue Header `h2` 4.5cqw → `display` 13cqw, weight 700–900) carries every statement.
...
- **Bebas Neue Header display is lowercase, weight 700–900, negative-tracked** (−0.04em largest, −0.02em h2). **Mono chrome is uppercase, 0.1em+.** No italic, no underline, no uppercase display.
...
Bebas Neue Header `display` (13cqw) lowercase in ink, left-anchored, over a small ink rule stub + mono kicker; a
Bebas Neue Header lead line beneath in 75% ink. **Chrome** mono catalogue number top-left, mono meta top-right
...
**Ground** ink-black. **Composes** kicker, display. **Focal** a 2–4 word Bebas Neue Header `display`/`h1`
...
row of three top-border-only stat-cards — big Bebas Neue Header-900 numeral in `{colors.fire-orange}`, Bebas Neue Header
...
**Ground** ink-black. **Composes** fadelist (3 stacked Bebas Neue Header-900 words at 1.0/0.5/0.22), fadelist-title.
...
Bebas Neue Header `quote-text` (700, lowercase) at ≤78cqw under an oversized fire-orange `quote-mark` (10cqw,
...
- Set every Bebas Neue Header display in **lowercase weight 900**, negative-tracked — the system's signature.
...
- Never uppercase Bebas Neue Header display; never add a second accent color.
...
- No serif companion; chrome is never Bebas Neue Header.
...
- **Type** — Bebas Neue Header lowercase 900 negative-tracked, fit-to-measure; mono chrome uppercase 0.14em; ≥1.4cqw floor.
...
- **Bebas Neue Header + IBM Plex Mono via Google Fonts**; Noto Sans SC is the CJK fallback (the lowercase-display signal has no CJK equivalent — the two-register color system carries the identity, per the source).
...
```html
<style>
@font-face{font-family:"Bebas Neue";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/BebasNeue-400.woff2") format("woff2");}
@font-face{font-family:"Bebas Neue";font-weight:400;font-style:normal;font-display:block;src:url("assets/fonts/BebasNeue-400.woff") format("woff");}
</style>
```
